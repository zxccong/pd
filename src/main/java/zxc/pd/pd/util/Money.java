...
public class Money implements Serializable, Comparable<Object> {
	...
	/**
	 * 多人平分金额
	 * @param money 平分的金额总数（分）
	 * @param userIds 平分的用户id
	 * @return Map key userid, value 平分后的金额 （分）
	 */
	public static Map<String,Long> averageMoney(Long money,List<String> userIds){
		Map<String,Long> averageMap = new HashMap<>();
		//rem  余数
		Long rem=money%userIds.size();
		//total 去除余数后能除断的金额
		Long total=money-rem;

		Random rand = new Random();
		//生成随机数  余数金额给随机到的人
		int random= rand.nextInt(userIds.size());
		for (int i=0;i<userIds.size();i++){
			//如果当前循环等于随机数，那么+余数金额
			if (i==random){
				averageMap.put(userIds.get(i),total/userIds.size()+rem);
			}else {
				averageMap.put(userIds.get(i),total/userIds.size());
			}
